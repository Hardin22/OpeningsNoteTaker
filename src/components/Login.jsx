...
const Login = ({ onClose, toggle }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setMessage('Accesso effettuato con Google!');
      if (onClose) {
        setTimeout(() => onClose(), 2000);
      }
    } catch (err) {
  ...
  return (
    <div className="text-white">
      <h2 className="text-2xl font-light mb-6 text-center tracking-wide">
        Accedi a <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600 font-medium">ChessNotes</span>
      </h2>
      ...
      <form onSubmit={handleLogin}>
        ...
        <div className="mb-6">
          <label htmlFor="password" className="block text-gray-300 text-sm mb-2 font-light">
            Password
          </label>
          <input
            type="password"
